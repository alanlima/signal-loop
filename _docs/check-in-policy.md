...
Unselected projects receive no submission, neutral score or inferred lack of concern. One combined journey per verified person per global Monday 00:00 UTC to next Monday 00:00 UTC is the UX limit, with at most three projects total across organisations. Each project retains its organisation-local eligibility/reporting week. The user approved this rule on 2026-09-07 for #13: submission, intentional discard or expiry seals the global week slot, including omitted projects; saving/leaving a valid draft does not. A new global week never permits duplicate contribution to an already-consumed project/local week. This draft constraint must not become a stored multi-project feedback identifier; [the #13 protocol](anonymous-submission.md) defines enforcement and verified-person prerequisites.
...
All five expire with the draft under #4 and the approved combined-journey rule in section 5: submission/discard deletes earlier than the frozen expiry. Example: Rowan writes `As the only overnight specialist, I handled the rare outage` in P4 and selects Cedar/Birch/Maple. All project payloads exclude that narrative. A stretched P2 does not fill any J2. Rowan can independently answer project workload differently. Do not suggest copying personal text into project fields.
...
Draft expiry is the earliest selected project-window close, global UTC journey-week end, or seven days after creation; submission/discard deletes it earlier. Begin freezes that expiry and section removal never extends it. Display the exact UTC expiry before Begin and on save/finish. A save acknowledgement means persistence succeeded; if save fails keep text visible, explain failure, permit retry and prevent accidental navigation without warning. A hard privacy expiry is pre-disclosed, warned about in an open draft and explicitly explained on late return; expired content must not be reconstructed. No automatic copying into next week.
...
| [#12](https://github.com/alanlima/signal-loop/issues/12) | Organisation-local Monday windows stored as UTC instants, compatible with #4 and earliest selected-window draft closure |
